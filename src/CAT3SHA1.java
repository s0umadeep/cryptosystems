...
public class CAT3SHA1 {

	public static int lengthMessage = 0;

	private static final int[] TABLE_T = new int[64];

	static {
		for (int i = 0; i < 64; i++)
			TABLE_T[i] = (int) (long) ((1L << 32) * Math.abs(Math.sin(i + 1))); // Modified code using MD5 added in
																				// place of K
	}

	public static String functionCtB(String word) {
	...
	public static void main(String[] args) {
		...
		System.out.println("The Clear Text is : " + word);

		String binary = functionCtB(word);
		lengthMessage = binary.length();
		ModCalculate(word, binary);

	}

	public static void ModCalculate(String word, String binary) {

		int binaryMessageLength = word.length() * 8 - 8;
		String endBitLength = MLCalculate(binaryMessageLength + 8);
		...
		MessageLengthCreate(binary, onePadded, binaryZeros, endBitLength);

	}

	public static String MLCalculate(int bitLength) {
	...
	private static String hash(int[] z) {
		...
		for (int i = 0; i < integer_count; i += 16) {
			...
			for (int x = 0; x <= 19; x++) {
				t = leftrotate(A, 5) + ((A & B) | (C & D)) + E + intArray[x] + k1; // Function F1 New Modified
				E = D;
				D = C;
				C = leftrotate(B, 30);
				B = A;
			...
			for (int b = 20; b <= 39; b++) {
				t = leftrotate(A, 5) + ((A & B) | (B & C) | (C & D)) + E + intArray[b] + k2; // Function F2 New Modified
				E = D;
				D = C;
				C = leftrotate(B, 30);
				B = A;
			...
			for (int c = 40; c <= 59; c++) {
				t = leftrotate(A, 5) + (((~A) & B) | ((~B) & C) ^ D) + E + intArray[c] + k3; // Function F3 New Modified
				E = D;
				D = C;
				C = leftrotate(B, 30);
				B = A;
			...
			for (int d = 60; d <= 79; d++) {
				t = leftrotate(A, 5) + (A ^ B ^ C ^ D) + E + intArray[d] + k4; // Function F4 New Modified
				E = D;
				D = C;
				C = leftrotate(B, 30);
				B = A;
		...
		System.out.println("The SHA for the following modified code is : " + hh);

		return null;
	}

	public static String MessageLengthCreate(String message, String onepad, int zeros, String endLength) {

		StringBuilder messageBinary = new StringBuilder(message);
		messageBinary.insert(messageBinary.toString().length(), onepad);
